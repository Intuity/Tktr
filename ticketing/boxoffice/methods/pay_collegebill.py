...
class PayCollegeBill(BaseLayout):

    @view_config(route_name="alter_pay_collegebill", context=Ticketing, permission="basic", renderer="templates/college_bill.pt")
    @view_config(route_name="pay_collegebill", context=Ticketing, permission="basic", renderer="templates/college_bill.pt")
    def college_bill_view(self):
        ...
        method = [x for x in methods if x.__name__ == "collegebill" and x.enabled == True]
        ...
        # - Now run some checks that this person actually should be able to pay this
            ...
            if not payment_id in self.request.root.payments:
                ...
                if payment.owner != self.user:
                    ...
                    return HTTPFound(location=self.request.route_path("user_profile"))
        # Get the payment method and processing parameters
        method = method[0]
        ...
        if payment_id == None:
            ...
            # Calculate subtotal, processing charges & total
            subtotal = 0
            for tick in tickets:
            ...
            return {
                "method": method,
                "more_details": method.get_value(PROP.COLLEGE_BILL_DETAILS),
            ...
            processing = process_fee
            total = subtotal + processing
            if payment.current_method == "collegebill":
